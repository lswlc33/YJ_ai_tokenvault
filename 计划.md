...
2. **内容计数**。一行四格：供应商 / 密钥 / 模型 / 平台账号，数字在上、标签在下。每格可点，跳管理页对应分段。这是"总览一切内容"最直接的一块，也顺带让用户看出"我到底存了多少东西"。
...
`ManageUiState(tab: ManageTab, loading, query, groupFilter, sort, providers, keys, models, accounts, selection: Set<Long>)`

`ManageTab = Providers | Keys | Models | Accounts`。用 `AppTabRow` 切换，**四类内容各自成表**——这是"所有内容的列表"的落地：用户想找一张密钥时不该先猜它挂在哪个供应商下面。

顶部固定区（不随列表滚动收起，只有 topBar 收起）：`AppSearchBar` + 分段 + 一行筛选/排序 chips。搜索作用于当前分段：

| 分段 | 搜什么 | 排序 |
| --- | --- | --- |
| 供应商 | 名称、备注、host、分组 | 名称 / 余额 / 最近探测 / 手动排序 |
| 密钥 | label、所属供应商名 —— **不搜密钥本身**（加密列，红线 3 的推论） | 健康状态 / 最近探测 / 所属供应商 |
| 模型 | modelId、显示名、所属供应商名 | 协议 / 名称 / 探测状态 |
| 平台账号 | 备注、所属供应商名 —— **不搜账号与密码本身** | 所属供应商 / 备注 |

行的样式（都用 `AppCard`，行高 ≥ `minTouchTarget`）：

- **供应商行**：左侧 `providers.color` 色块 + 首字母 → 名称 + 置顶标 → 备注 → host → 协议 chips；右侧 Key 数 / 可用数 / 模型数 / 余额 / `StatusDot`。有平台账号时加一个小钥匙角标。`StatusDot` 走 §5.3 的聚合规则（有一张可用就算可用）。
- **密钥行**：label + 所属供应商名（次行）+ 遮蔽串（`SecretText` 的折叠形态，**列表页只显示遮蔽串，不解密**——遮蔽串是详情页解密后现算的，所以列表页这一列存的是入库时算好的静态遮蔽串，见 §6.1 推论 3）+ `StatusDot` + 延迟 + `RelativeTime`。默认那张带"默认"标。
- **模型行**：modelId（等宽字体）+ 显示名 + 协议 chip + 来源标（手动 / 发现）+ 探测状态 + 上下文/输出/单价。停用的行整行降到 `muted` 色并在右侧标"已停用"。
- **平台账号行**：备注 + 所属供应商 + 账号的遮蔽串 + "仅本地记录"角标。**不在列表里显示密码的任何形态**，连遮蔽串都不给——账号和密码同等对待（红线 21），但列表页连账号都只给遮蔽串，密码只在详情页展开。

交互：

- 点行 → 详情（供应商行进 `ProviderDetailRoute`；密钥 / 模型 / 账号行进所属供应商详情并滚动定位到那一行，**不为它们单独做详情页**——它们没有独立于供应商的意义）。
- 长按进多选：批量探测 / 批量分组 / 批量启用停用 / 批量删除。删除一律二次确认，文案里写清连带删掉什么。
- FAB：`OverlayListPopup` 两项——「手动新建供应商」「粘贴导入」。在密钥 / 模型 / 账号分段时，FAB 第一项变成"给某个供应商添加…"，先选供应商再进编辑器。
- 顶栏 actions：分组管理（进 `GroupsRoute`）、全选/退出多选。
- **不放下拉刷新**：管理页是编辑页，下拉刷新在这里的语义含糊（刷新列表？重新探测？）。要探测就去仪表盘那个明确的按钮。

空态按分段给不同文案："还没有供应商"给两个 CTA；"还没有密钥"提示先建供应商。
...
- *探测*（`ProbeSettingsRoute`）：自动探测时机、是否启用 L3、**升级版 L2 是否允许消耗额度**（8.3）、超时、host 最小间隔与每轮请求预算（8.1、8.5）、余额阈值（按币种）、客户端预设自动嗅探开关、客户端拦截关键词编辑、详细 HTTP 日志（默认关 + 风险说明）、手动代理。
...
| **M0.8** | 界面骨架与三页外观 | **底栏改为仪表盘 · 管理 · 设置**（§13.1），补齐全部路由与二级页壳；三个一级页按 §13.4 做出**完整外观**——仪表盘六块卡、管理页四个分段与四种行样式、设置页四块导航 + 五个二级设置页 + 同步页 + 更新页；MIUIX 包装层补到够用（`AppTabRow` / `AppSearchBar` / `AppRefreshBox` / `AppSwitchRow` / `AppDropdownRow` / `AppBottomSheet` / `AppListPopup` / `AppChip` / `AppProgress` / `AppSegmentedBar`）；`ui/common` 补 `StatusDot` / `SecretText` / `RelativeTime` / `ErrorState` / `LoadingBox`。数据先用 `screens/sample/SampleContent.kt` 喂**样例数据**（M3 换成 ViewModel + 仓库时整个 `screens/sample/` 包删掉；页面只接 UiState 与回调，本来就不知道数据从哪来，所以换源不动布局） | 模拟器上三个一级页与每个二级页都能进能回；深浅色各截一轮；中英文各截一轮；`OverlayDialog` / `OverlayBottomSheet` 在二级页里都能弹；`ArchitectureRulesTest` 仍全绿（无中文字面量、无 `Color(0xFF…)`、无裸 `fontSize`、`ui/miuix` 之外无 MIUIX import） |
