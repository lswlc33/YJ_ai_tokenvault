...
| 项 | 值 |
| --- | --- |
| 版本 | v3.1（Android 原生重写版；v3 评审后修正） |
| 日期 | 2026-09-04 |
| 输入 | `示例数据.md`、三个参考项目、models.dev / DeepSeek / new-api / MIUIX 官方文档 |
| 状态 | 除第 18 节的 2 项外，其余决策已定，可直接开工 |
...
- MIUIX 当前版本 **0.9.2**，坐标 `top.yukonga.miuix.kmp:miuix-ui`（Android 加 `-android` 后缀），已拆成 `miuix-ui` / `miuix-preference` / `miuix-icons` / `miuix-blur`（要求 minSdk 33）/ `miuix-squircle` / `miuix-nav` 六个可独立引入的模块。官方标注**实验阶段，API 可能无预告变更**。
...
- MIUIX 0.9.2 自带 `Snackbar` / `SnackbarHost` / `SnackbarHostState` / `SnackbarDuration` / `SnackbarResult`（`top.yukonga.miuix.kmp.basic`），挂在 `Scaffold(snackbarHost = …)` 上。因此**不需要为了 Snackbar 引入 `material3`**（Scaffold 那一页文档里"Miuix 不提供此组件"的说法已过时，以 Snackbar 组件页为准）。
...
编号 1–23 来自 v3，24–32 是 v3 评审后补的。**编号只作稳定标识，不代表优先级或顺序**，所以后补的条目直接挂在各组末尾而不重排。
...
- **29.** **探测不允许把自己探成失败**。单个供应商单轮的请求次数必须有上限（基线 + 鉴权风格重试 + L2 + 预设嗅探叠起来很容易撞上中转站的限流），一旦本轮出现 429 就停止该 host 的后续可选请求（尤其是预设嗅探），而不是继续试下一个。
- **30.** **发现来的模型必须记住它是从哪个协议的列表里来的**。`GET /v1/models` 不返回协议，所以"上游消失即停用"的作用域只能限定在**本轮实际查询过的那个协议**，否则只拉了一个协议的列表就会把别的协议下的发现项全部停用。
...
- **32.** **落库前的脱敏以"已知明文值替换"为主，正则只作兜底**。正则挡不住任意形态的凭据——`EXAMPLEtoken0000/EXAMPLE0000000=` 这种 base64 访问令牌不匹配任何一条 `sk-` / `Bearer` 规则。任何要落库或落日志的字符串，先把当前会话已知的密钥 / 令牌 / 密码明文（及其 ≥8 字符前缀）替换成 `<已省略>`，再过正则。
...
| 层 | 选择 | 理由 |
| --- | --- | --- |
| 语言 / 工具链 | Kotlin 2.x，JDK 17，AGP 8.x，KSP | 与 `photo` 一致 |
| SDK | minSdk 33 / compileSdk 37 / targetSdk 37 | `miuix-blur-android` 要求 33；与 `photo` 一致 |
| UI | Jetpack Compose + **MIUIX 0.9.2** | 不引入 `material3`，避免两套主题打架 |
...
app/src/main/java/com/lc33/tokenvault/
  ...
  di/                    Hilt Module
  ui/
    theme/               MiuixTheme 包装、AppTokens、StatusPalette
    miuix/               MIUIX 包装层（AppScaffold / AppCard / AppRow / AppDialog …）
...
`https://openrouter.ai/api/v1` 这类带子路径的也正确：剥掉尾部 `v1` 得 `https://openrouter.ai/api`，拼回去还是对的。DeepSeek 的 Anthropic 兼容端点在 `/anthropic` 下——**不要用启发式猜，让用户覆盖**，这就是 `pathOverrides` 存在的理由。
...
| 面 | 措施 |
| --- | --- |
| 日志 | 自写 OkHttp `RedactingInterceptor`：请求头里 `Authorization` / `x-api-key` / `New-Api-User` 一律记为 `<已省略>`；**响应体默认不记**，仅在设置里显式打开"详细 HTTP 日志"时记录并截断到 2 KB。落库前统一过 `Redactor.scrub()`，两道（红线 32）：**第一道按已知明文值替换**——把当前会话已知的密钥 / 访问令牌 / 平台密码 / WebDAV 密码及其 ≥8 字符前缀全部换成 `<已省略>`；**第二道才是正则兜底**——`sk-[A-Za-z0-9_\-]{16,}`、`Bearer\s+\S+`、`"(token\|key\|secret\|api_key\|access_token)"\s*:\s*"[^"]*"`、邮箱。只有正则时挡不住 `EXAMPLEtoken0000/EXAMPLE0000000=` 这种 base64 访问令牌，而它恰好是最敏感的一类。**不使用 `HttpLoggingInterceptor`**（它会打全量 body）。`Redactor` 必须有单测，用例里要包含 base64 令牌。 |
...
| `claude_code` | Claude Code | anthropic | `claude-cli/1.0.119 (external, cli)` | `x-app: cli`、`anthropic-beta: claude-code-20250219,oauth-2025-04-20`、`x-stainless-lang: js`、`x-stainless-runtime: node`、`x-stainless-runtime-version: v22.14.0`、`x-stainless-os: MacOS`、`x-stainless-arch: arm64`、`x-stainless-package-version: 0.60.0`、`x-stainless-retry-count: 0` | 中 |
...
探测结果为 `CLIENT_BLOCKED` 时，**手动**探测会按协议匹配依次重试内置预设（最多 4 个），命中则写回 `providers.clientProfileId`、置 `verified = 1`，并记一条日志"已自动匹配客户端预设：Codex CLI"。自动探测不做嗅探（省额度）。开关 `settings.sniffClientProfile`，默认开。
...
`CLIENT_BLOCKED` 的识别关键词（存 `app_settings`，用户可编辑）：`客户端`、`Claude Code`、`Codex`、`不支持该客户端`、`invalid client`、`client not allowed`、`user-agent`、`UA 校验`、`forbidden client`。命中时判 `CLIENT_BLOCKED` 而不是 `FORBIDDEN`，UI 直接给出"去换客户端预设"的按钮。
...
| 级 | 名称 | 粒度 | 请求 | 成本 | 默认 |
| --- | --- | --- | --- | --- | --- |
| L1 | 连通性 / 模型列表 | 供应商 × 协议 | `GET {models}` | 0 | 开 |
| L2 | 密钥有效性 | 每张 Key | 用该 Key 发 L1；若该站 models 路由不鉴权，则**升级为真实极简推理调用** | 0 或 ~16 token | 开 |
| L3 | 模型可用性 | 每个模型 | 按 `model.protocol` 发极简推理调用（5.2 表 + 8.2 的客户端预设） | ~16 token / 模型 | 关 |
| L4 | 余额 | 供应商 | 见第 9 节 | 0 | 有配置就开 |

**成本要说准**：L2 在"该站 models 路由不鉴权"时会升级成真实推理调用，所以"自动探测永不包含 L3"并不等于自动探测零消耗。设置页里那一行说明要写成"自动探测通常不消耗额度；若某站的模型列表接口不鉴权，密钥验证会退化为一次约 16 token 的真实调用"，并提供 `settings.autoProbeAllowUpgradedL2`（默认开）让在意的人关掉。

**不鉴权基线检测**（红线 12 的落地）：先用 `Authorization: Bearer yj-probe-invalid` 发一次 L1。若也返回 200，说明这家的 models 路由不鉴权，此后该 host 的 L2 自动升级为真实调用，并在详情页标注"该站模型列表接口不鉴权，有效性以实际调用为准"。基线结果按 host 缓存在 `app_settings`，24 小时有效。
...
**判定顺序很重要，按下表从上往下短路**。两条容易搞反的原则先写在这里：
...
| 3 | 任意状态码，且响应体命中**客户端校验关键词**（见 8.2） | `CONCLUSIVE_FAIL` | `CLIENT_BLOCKED`，手动探测时触发预设嗅探 |
| 4 | 402，或任意 4xx/5xx 且 message 命中 `insufficient\|quota\|balance\|额度\|余额\|欠费\|已用尽` | `CONCLUSIVE_FAIL` | `INSUFFICIENT` |
| 5 | 401 | `CONCLUSIVE_FAIL` | `UNAUTHORIZED` |
| 6 | 403 | `CONCLUSIVE_FAIL` | `FORBIDDEN` |
| 7 | 429 | 读 `Retry-After`，指数退避重试至多 2 次；仍 429 → `RATE_LIMITED` | **否** |
| 8 | 5xx | 幂等请求（GET）重试 1 次；仍失败 → `UPSTREAM_ERROR` | **否** |
| 9 | 400 且上游 message 提到 `max_tokens` | 换 `max_completion_tokens` 重试一次；仍失败 → 走第 10 行 | — |
| 10 | 400（L2） | `SUCCESS` | `OK`，`healthDetail` 记"密钥有效，但请求参数不被接受：…" |
| 11 | 400（L1 / L3） | `CONCLUSIVE_FAIL` | `CONFIG_ERROR`，detail 带上游 message 前 200 字符 |
| 12 | 404（模型级 L3） | `CONCLUSIVE_FAIL` | 模型置 `NOT_FOUND` |
| 13 | 404（供应商级 L1） | `CONCLUSIVE_FAIL` | `CONFIG_ERROR`「路径不对，检查 API 地址与路径覆盖」 |
| 14 | 2xx 且能解析出预期字段 | `SUCCESS` | `OK` + 延迟 + `okAt = now` |
| 15 | 2xx 但内容不是预期结构（HTML 登录页、`{"error":…}`） | `CONCLUSIVE_FAIL` | `CONFIG_ERROR`「端点返回了非预期内容」 |
| 16 | 本轮预算耗尽 / 撞 host 预算 / 金库已锁定 | `SKIPPED` | 不写任何字段 |
...
手动：首页下拉刷新 / 顶栏按钮（全量）、详情页刷新（单供应商）、Key 行长按（单 Key）、模型行点击（单模型）。

自动（默认全部关闭，避免无声消耗额度）：解锁后探测、定时（1h / 6h / 1d，用 WorkManager 周期任务 + `NetworkType.UNMETERED` 约束）、仅 Wi-Fi。**自动探测永不包含 L3，也不做客户端预设嗅探**；升级版 L2 会消耗少量 token，由 `settings.autoProbeAllowUpgradedL2` 控制（8.3）。金库锁定时 Worker 直接返回 `Result.success()` 并记日志，下次解锁后补做。
...
| `deepseek` | DeepSeek 官方 | `GET {apiRoot}/user/balance`，Bearer API Key | `balance_infos[0].total_balance`（**字符串，要 parse**），币种取同条的 `currency`；多币种时取第一条并在 UI 提示 | 上游给 |
| `openrouter` | OpenRouter | `GET {apiRoot}/{ver}/credits`，Bearer API Key | `data.total_credits - data.total_usage` | USD |
...
@Serializable data object ProbeRoute
...
@Serializable data object WebDavRoute
...
一级页三个：**金库 · 探测 · 设置**（日志降为设置的二级页）。宽度 ≥ 600dp 时 `bottomBar` 换成 `NavigationRail`，且金库页变列表/详情双栏（详情不再作为独立目标压栈）；≥ 1000dp 内容区限宽居中。tab 索引存 `SavedStateHandle`，锁定再解锁后保持。
...
@Composable
fun AppTheme(mode: ColorSchemeMode, content: @Composable () -> Unit) {
    val controller = remember(mode) { ThemeController(mode) }
    MiuixTheme(controller = controller) {
        CompositionLocalProvider(
            LocalAppTokens provides AppTokens.Default,
            LocalStatusPalette provides statusPaletteFor(isSystemInDarkTheme()),
            content = content,
        )
    }
}
...
**金库（首页）** — `VaultUiState(loading, items: List<ProviderCard>, totals: Map<Currency, Double>, summary, query, groupFilter, sort, selection: Set<Long>)`
- 汇总卡：供应商数 / Key 数 / 可用 Key 数 / 模型数 / **按币种分组的总余额**。
- 工具条：`AppSearchBar`（搜名称、备注、host、模型 id、账号备注——**不搜账号本身**，它是加密列）、分组筛选、排序（名称 / 余额 / 最近探测 / 手动）。
- 供应商卡：色块首字母 → 名称 + 置顶标 → 备注 → host → 协议 chips → 右侧 Key 数 / 可用数 / 模型数 / 余额 / `StatusDot`；有平台账号时加一个小钥匙角标。`StatusDot` 走 5.3 的聚合规则（有一张可用就是可用），`lastOutcome` 为瞬时类时在角标上体现"本轮未验证"。
- 长按进多选：批量探测 / 批量分组 / 批量删除。
- `AppRefreshBox` 下拉触发全量探测；FAB 两个入口（手动新建、粘贴导入）。
- 空态给可点的 CTA。

**供应商详情** — `DetailUiState(provider, keys, accounts, modelsByProtocol, balance, clientProfile, catalogHits, revealedIds)`
...
**供应商编辑** — `EditorUiState(draft, endpointPreview, validation, dirty)`
...
**客户端预设编辑** — `ProfileEditorUiState(draft, parsedFromCurl, strippedHeaders)`：名称、`User-Agent`、有序头部键值表（可增删拖拽）、`bodyPatch`（JSON 编辑框 + 校验）、适用协议多选；顶部"从 cURL 导入"按钮 → 粘贴框 → 预览（显示解析出的头部与**被剔除的头部及原因**）→ 应用。底部"用某个供应商试一下"直接发一次 L2 探测看结果。

**导入** — `ImportUiState(rawText, parsed: List<ParsedProvider>, issues, selected)`：粘贴框（含"从剪贴板填充"）→ 预览列表（每条可勾选、可展开编辑、带校验徽标）→ 确认写入。

**探测** — `ProbeUiState(lastRun, progress, results: List<ProbeItemResult>, failedOnly)`：上次摘要（时间 / 总数 / 成功失败 / 耗时）、本轮进度 + 取消 + "运行中不会自动锁定"的说明（7.4）、按结果分组的清单（失败项、以及**本轮未探测项**，可展开看请求概要与上游 message）、"仅重试失败项 / 未探测项"、范围与级别开关（L1–L4）。

**设置** — 用 `miuix-preference` 的行组件，分七组：
- *外观*：配色模式（六选一）、圆角风格（squircle 开关）、底栏模糊（`miuix-blur`，可关）。
- *安全*：生物识别解锁、自动锁定时长、前台空闲锁定、屏幕关闭即锁定、修改 PIN / 改用口令、**恢复密钥（查看提示 / 重新生成）**、失败自毁次数、展示密钥时防截屏、剪贴板自动清除时长。
- *探测*：自动探测时机、是否启用 L3、**升级版 L2 是否允许消耗额度**（8.3）、超时、余额阈值（按币种）、客户端预设自动嗅探开关、客户端拦截关键词编辑、详细 HTTP 日志（默认关 + 风险说明）、手动代理。
- *客户端预设*：预设列表（内置 + 自定义，显示是否已验证）、新建 / 从 cURL 导入、恢复内置预设为默认值。
- *数据*：模型元数据（上次同步 + 立即同步 + 数据源地址）、分组管理、清空探测结果（二次确认）、日志保留条数与天数、清空日志（二次确认）。
- *备份*：导出加密备份、导入备份、**备份口令（默认沿用 PIN，旁边常驻"要上云就单独设长口令"的提示）**、WebDAV 设置、自动备份。
- *关于*：版本（读 `BuildConfig`）、做什么 / 不做什么、**加密边界表**（4.4）、威胁模型（7.6，含"6 位 PIN 挡不住离线穷举"与"别在这里存复用过的密码"）、"这不是密码管理器"说明（2.3）、第三方开源组件与许可证。
...
单模块 `:app`，Gradle wrapper（Windows 用 `.\gradlew.bat`），版本全部锁在 `gradle/libs.versions.toml`。下表只给坐标；除已核实的 MIUIX 0.9.2 外，版本取开工当日 Maven Central 稳定版并写死，不用动态版本。

| 用途 | 坐标 |
| --- | --- |
| 插件 | `com.android.application`、`org.jetbrains.kotlin.android`、`org.jetbrains.kotlin.plugin.compose`、`org.jetbrains.kotlin.plugin.serialization`、`com.google.devtools.ksp`、`com.google.dagger.hilt.android` |
| 插件 | `com.android.application`、`org.jetbrains.kotlin.android`、`org.jetbrains.kotlin.plugin.compose`、`org.jetbrains.kotlin.plugin.serialization`、`com.google.devtools.ksp`、`com.google.dagger.hilt.android`、`androidx.room`（Room Gradle 插件，用来声明 schema 目录） |
| UI | `top.yukonga.miuix.kmp:miuix-ui-android:0.9.2`、`miuix-preference-android`、`miuix-icons-android`、`miuix-squircle-android`、（可选）`miuix-blur-android` |
| Compose | `androidx.compose:compose-bom` → `ui`、`ui-graphics`、`foundation`、`ui-tooling-preview`；`androidx.activity:activity-compose`；`androidx.fragment:fragment-ktx` |
...
| 数据库 | `androidx.room:room-runtime`、`room-ktx`、`room-compiler`(ksp)、`room-testing`(androidTest) |
| 网络 | `com.squareup.okhttp3:okhttp`；`com.squareup.okhttp3:mockwebserver`(test) |
...
| 加密 | `org.bouncycastle:bcprov-jdk18on`、`androidx.biometric:biometric` |
...
1. **URL 规范化**：5.2 表三条示例数据 + 无 scheme + 带端口 + 带子路径 + 多余斜杠 + `v1beta` + **带 query 必须报错** + 非法输入。
2. **协议请求构造**：三种协议的 method / URL / 头 / body 逐字段断言（含 `max_tokens = 16`）；用 `MockWebServer` 验证真实发出的报文。
3. **错误分类**：8.4 矩阵每一行，**尤其是顺序**——`403` + `该令牌额度已用尽` 必须判 `INSUFFICIENT` 而不是 `FORBIDDEN`，`400` + `insufficient user quota` 同理；`400`（L2）判 `OK` 而 `400`（L1/L3）判 `CONFIG_ERROR`；429 带 `Retry-After`；400 的 `max_tokens` 降级重试；`UnknownHostException` / 429 / 5xx **都不改写 health**；客户端拦截关键词命中 `CLIENT_BLOCKED` 而不是 `FORBIDDEN`。
4. **余额适配器**：每个内置适配器用真实响应样例断言 `BalanceSnapshot`——DeepSeek 的字符串金额、NewAPI 的 quota÷500000、OpenRouter 的减法；解析失败必须抛异常而不是返回 0；金额格式化与按币种求和不能出现浮点尾数。
...
7. **脱敏**：`sk-`、`Bearer `、JSON 里的 `token` / `key` / `secret` 值都必须被打掉；**base64 形态的访问令牌（形如 `EXAMPLEtoken0000/EXAMPLE0000000=`）必须靠"已知明文值替换"这一道被打掉**（红线 32）。这是防泄漏的最后一道闸。
...
14. **探测编排**：`runTest` 虚拟时间验证进度推流、取消真的停、总预算超时后剩余项标 `SKIPPED`、`CLIENT_BLOCKED` 后按协议顺序嗅探预设且最多 4 次、**本轮出现 429 后立即停止该 host 的嗅探与其它可选请求**（红线 29）、**每 host 请求预算生效**。
...
- **`ci.yml`**（push / PR）：JDK 17 + Gradle 缓存 → `:app:testDebugUnitTest` → `:app:lint` → `:app:assembleDebugAndroidTest`（只编译，不跑）→ `:app:assembleNightly`；master 上把同一个 APK 刷新到 `nightly` 预发布。另加一组 `grep` 步骤守住架构规则：
...
- **`CLAUDE.md`**：架构 + 第 3 节那 32 条红线 + 6.1 的四条推论（Room 是应用级单例、锁定只清 DEK 不关库、DAO 不解密、boot 原子写）+ 4.4 的加密边界表。这些是改坏了不会立刻报错的东西。
...
| **M0.5** | 协议踩点（一次性 spike） | 一个 `src/test` 下的临时 `main()`（或一个带 `@Ignore` 的测试），用裸 OkHttp 打 `示例数据.md` 里那三家真实中转站：`/v1/models` 带 / 不带鉴权、极简推理调用、故意用错的 Key、故意用不存在的模型、`/api/user/self` 与 `/api/status`、不带 UA 与带各内置预设 UA 各来一次。把**原始状态码与响应体**存成 `test/resources/fixtures/*.json` | 拿到答案：这三家的 models 路由是否鉴权、额度耗尽返回什么码和什么文案、是否按客户端指纹拦、`quota_per_unit` 实际是多少、UA 影响多大。这些答案直接改写 8.4 的矩阵、9.2 的适配器和 8.2 的预设表——放到 M5–M7 才发现就要返工四个里程碑 |
...
| **M3** | 金库 UI | 列表 / 详情 / 编辑三页 + 平台账号区 + 快速复制面板，MIUIX 包装层成型（只含 `Overlay*`），`SecretText`、剪贴板策略、URL 规范化实时预览、分组与色块、分组管理 | 测试 1 全绿；能手工录完 `示例数据.md` 三条数据并正确显示三套协议 URL；账号密码的遮蔽 / 回遮 / 清剪贴板与密钥完全一致；每个持久化字段都能在 UI 里找到入口 |
...
| **M10** | 打磨与发布 | `audit_log` 持久化 + 脱敏 + 日志页、搜索 / 排序 / 批量、宽屏双栏、无障碍、strings 双份、release 签名、nightly 与 tag 两条发布链路、release 包全流程回归 | `lint` 无 error；TalkBack 走通主路径；nightly APK 可覆盖安装；release 包不崩在序列化上 |
| **M11** | 可选增强 | ①供应商预设库（借鉴 cc-switch）②Deep Link / 二维码导入 ③SQLCipher 整库加密（4.4 的可选项）④桌面端（需先把 UI 抽到 Compose Multiplatform，成本较高） | 逐项独立评估 |

M0 之后立刻做 M0.5，它半天到一天，但决定 M5–M7 的正确性。M0–M3 是地基，建议连续做完；M5 与 M6 强耦合，最好同一批做完再收；M7 的六个适配器可以并行验证。
...
- [ ] 日志里搜不到 `sk-`、搜不到 `Bearer ` 后跟内容、**搜不到 base64 形态的访问令牌**、搜不到平台密码（红线 32）
...
- [ ] 故意写错的 Key → "密钥无效"；不存在的模型 → "模型不存在"；两者不混
- [ ] 额度耗尽的中转站（返回 403 或 400 + 额度文案）显示"余额不足"，不是"无权限"或"配置错误"
...
1. **`zcode` 及其它你常用客户端的真实请求头**。我没有可靠的 `zcode` 指纹资料，内置预设里先留空位。请在电脑上抓一次它的请求（mitmproxy / Charles 均可）并把 cURL 发我，或者等应用做完直接用 8.2 的 cURL 导入自己补——后者其实更省事，因为你还能顺手校准 Claude Code / Codex 的版本号。如果还有别的客户端要内置（Cline、Roo、Kilo、Cherry Studio 之类），一并列出来。
