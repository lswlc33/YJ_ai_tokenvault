 ...
 * 金额 → 展示串。
 *
 * **先定点舍入再展示**（§9.1）：`FormatMoney.roundedCents` + `centsToPlainString` 到两位。
 * 直接把 `Double` 拼进字符串的表现是首页出现 `42.099999999999994`。
 * 币种不在这里拼——它是 [UiMoney] 的独立字段，因为首页按币种分组且不做汇率换算（红线 15）。
 */
fun formatAmount(amount: Double): String =
    FormatMoney.centsToPlainString(FormatMoney.roundedCents(amount))
 ...
 *
 * 阶段3：此函数接收 `ProbeRunEntity`（Room 实体），留在 app 侧（`ui/shell/` 的
 * `ProbeRunMapping.kt`），因为 Room 实体是 data 层专属。这里不重复定义。
 */
