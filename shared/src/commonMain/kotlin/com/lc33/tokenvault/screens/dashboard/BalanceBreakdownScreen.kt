...
import tokenvault.shared.generated.resources.balance_empty_desc
import tokenvault.shared.generated.resources.balance_failed_hint
...
fun BalanceBreakdownScreen(
    ...
    AppScaffold(
        ...
        if (byCurrency.isEmpty() && failedProviders.isEmpty()) {
            EmptyState(
                title = stringResource(Res.string.dashboard_balance_none),
                description = stringResource(Res.string.balance_empty_desc),
        ...
        LazyColumn(
            ...
            if (failedProviders.isNotEmpty()) {
                item { SectionTitle(text = stringResource(Res.string.balance_failed_section)) }
...
private fun FailedRow(row: UiProviderRow, onOpen: (Long) -> Unit) {
    ...
    AppCard(
        ...
        // "查不到"与"是 0"必须能分开（§9.3）
        AppText(
            text = stringResource(Res.string.balance_failed_hint),
            style = AppTextStyle.Footnote,
            color = palette.warn,
