...
/**
 * 详情页与供应商卡片里那两块模型预览的行数上限。
 *
 * 外面只作**名称参考**：一眼看清这把 Key 有哪些模型，要看协议、上下文、能力与
 * 增删改都进整屏模型页。三行是"能认出这一家的模型长什么样"的最小样本，再多就是
 * 把那一页的内容抄到卡片里，而一屏几十行正是那张卡原先撑到几屏高的原因。
 */
const val MODEL_PREVIEW_LIMIT = 3
