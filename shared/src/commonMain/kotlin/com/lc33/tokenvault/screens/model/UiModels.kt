 ...
 * 而这一卡的作用是认出这把 Key 通了哪些模型，不是读模型清单。
 */
const val PROVIDER_MODEL_PREVIEW_LIMIT = 2

/**
 * Key 详情页模型卡的预览行数。
 *
 * 这一页只有一把 Key，摊到十行不挤；再多就是整屏模型页的活（分组、搜索、能力、增删改）。
 * 两处都只作**名称参考**，末尾那行「查看全部模型」才是入口。
 */
const val KEY_MODEL_PREVIEW_LIMIT = 10
