...
object PinPolicy {
    ...
    /**
     * 默认长度，也是**当前唯一的长度**：解锁 PIN 固定 6 位数字，应用内没有"改成长口令"的入口。
     *
     * 旧注释写着"设置里可以换成任意长度口令"，而那一档从来没做出来——文案与能力不一致的
     * 代价是具体的：用户会在引导页读到"可以改用口令"，然后在设置里找不到它。
     * 真要支持长口令，得同时改这一行、`strings.xml` 里那两句说明、以及锁屏的键盘形态。
     */
