 ...
 * 上限 8s。
 *
 * 这里不做并发控制（那是引擎的活），只管"同一 host 相邻两个请求之间的最小间隔"：
 * `acquire(host)` 挂起直到距离上一个该 host 的请求过去了足够久。
 *
 * @param nowMillis 注入的单调时钟，测试用虚拟时间。**无默认值**：当前时间是平台能力
...
class HostGate(
     ...
     *
     * 真正的 `delay` 由调用方负责——这里只记账，保持纯逻辑可测。
     */
