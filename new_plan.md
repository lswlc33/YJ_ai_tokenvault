...
>
> 状态截至 2026-09-08。代码基线：提交 `8cf715d`（M0–M10 功能层已全部落地并提交，
> 其后 12 个提交是 CI 收尾，另有「撑谎」清理一批提交）。剩余工作见 §3——全是
> "要设备 / 要额度 / 要硬件"的验收，加上一个可砍项（M8 WorkManager 拉取；
> M10 宽屏双栏已决定不做）。
>
> **2026-09-08 核对后的三点修正**（下表与 §3 均已同步）：
> 1. M5 探测与 M7 余额的设备端到端**已于 09-07 跑通**，ADB 端到端只剩 M6 客户端伪装。
> 2. 「生物识别设备验证」**作废**——双端迁移阶段1 已删除生物识别，见 `迁移计划.md`。
>    同理，本文件与 `CLAUDE.md` 中关于恢复密钥、`FLAG_SECURE`、Argon2id 的叙述都是迁移前的历史。
> 3. 新增 P1：**iOS 真机验收**（迁移阶段4 的收尾项），详见 `迁移计划.md`。
...
| M1 安全底座 | ✅ | ~~生物识别无设备可验~~ **该功能已随双端迁移阶段1 删除，此项作废**；**前台空闲锁定 + 屏幕关闭即锁定已落地**（`app_settings` 可编辑，探测/备份进行中自动挂起空闲锁定） |
...
| M5 探测引擎 | ✅ | **设备端到端已跑通（09-07，Agent Router：L1+L2 全过、258 毫秒）**；剩 L3 逐模型探测未实现（要花钱、仅手动，见 §2） |
| M6 客户端伪装 | ✅ | 仅剩设备端到端（Agent Router 三路径 / TLS 指纹终止结论）；**拦截关键词设置已落地**（`app_settings` 可编辑） |
| M7 余额 | ✅ | **设备端到端已跑通（09-07，$119.02，与后台一致）**；剩各适配器贴真实响应样例；**余额阈值设置已落地**（`app_settings` 可编辑，USD/CNY）；**手动代理设置已落地**（`app_settings` 可编辑，`host:port`，支持 IPv6 方括号） |
| M8 模型元数据 | 🟡 纯逻辑层已做 | WorkManager 拉取 `api.json` 未做（可砍） |
| M9 备份与同步 | ✅ | 第二台设备恢复验证未做；WebDAV 半边可砍 |
| M10 打磨与发布 | ✅ | TalkBack 未做（需设备）；**搜索/排序/批量、更新页 API 已落地**；宽屏双栏已决定不做 |
| M11 可选增强 | ➖ | 逐项独立评估 |

**功能层（M0–M10 的代码、测试、UI 接线）已全部落地并提交**。剩余的都是"要设备 / 要额度 /
要硬件"的验收（M6 客户端伪装、iOS 真机、额度耗尽采集、TalkBack），以及一个可砍项
（M8 的 WorkManager 拉取；M10 的宽屏双栏已决定不做，2026-09-07），不再是功能缺失。
...
- **未验项**：~~生物识别（当前设备报无硬件，安全页那一行是禁用态）~~ **已随双端迁移阶段1 删除，此项作废**。
...
- **缺口**：~~ADB 端到端~~（✅ 09-06 已跑通：粘贴 → 解析预览 → 确认 → 落库 → 详情页解出明文）；SAF 导出入口（可砍，§4.7）。
...
- **缺口**：~~设备端到端~~（✅ 09-07 已跑通：`New-Api-User` 头配好后查余额得 **$119.02**，与后台一致）+ 每个适配器贴真实响应样例。
...
- `audit_log` 持久化 + 日志页（条数 + 天数双重上限）；~~README（如实写 6 位 PIN 挡不住离线穷举）~~ README **已于 2026-09-08 重写为用户向版本**，原「6 位 PIN 挡不住离线穷举」整段随安全定位降级为「记事本级」而删除。
- 发布链路：ci.yml 单测+lint+仪器编译+`assembleNightly` 一次 gradlew；keystore 走 secret；
  nightly 预发布走 `nightly-build` tag（仓库规则禁建 `nightly` tag）。
- **缺口**：TalkBack 走通主路径。搜索/排序/批量、更新页（GitHub Releases API）已落地；宽屏双栏已决定不做。

---

## 3. 未完成清单（按优先级）

**功能层已全部落地**。下表剩余项分三类：要设备/额度/硬件的验收、双端迁移收尾的
iOS 真机验收，以及一个可砍项（M8 WorkManager 拉取；宽屏双栏已决定不做）。

| 优先级 | 项 | 说明 | 粗估 |
| --- | --- | --- | --- |
| **P1** | iOS 真机验收 | 迁移阶段4 的收尾项：未签名 IPA 签名安装后走一遍 锁屏 → 引导 → 新建 → 导入 → 探测 → 备份。已知边界：iOS 无前台空闲锁定与屏幕关闭锁定的公开 API，用进后台兑底；boot 原子写不 fsync。见 `迁移计划.md` | 需 Mac + 签名证书 |
| **P1** | M6 客户端伪装设备端到端 | Agent Router 三条路径 + JustDoWork 空 body + TLS 指纹终止结论。**M5 探测与 M7 余额已于 09-07 在设备上跑通**（真实额度，见 §2） | 需设备 + 真实额度 |
| **P2** | 仪器测试 | ✅ 完成：`VaultDatabaseTest`（6 用例：`idx_keys_default` 部分唯一索引、外键 CASCADE/SET_NULL）+ `FileBootStoreTest`（JVM）。`SecureFlagTest` 随迁移阶段1 删除 `FLAG_SECURE` 一并移除。还差：Room 迁移测试（升 v2 时补） | ✅ 完成 |
| **P2** | TalkBack 无障碍 | 走通主路径 | 1 人日，需设备 |
| **P2** | 额度耗尽真实响应 | DeepSeek CNY 0.89 耗尽后补一次 `ProtocolSpike` 进 `fixtures/probe-matrix.json` | 余额自然耗尽时 |
| **P2** | M8 WorkManager 拉取 | 可砍，不影响核心四件事 | 2 人日 |
| **P3** | M11 可选增强 | 供应商预设库 / Deep Link / SQLCipher / 桌面端 | 逐项评估 |
| — | ~~生物识别设备验证~~ | **作废**：双端迁移阶段1 已删除生物识别（连同恢复密钥、独立备份口令、`FLAG_SECURE`） | — |

> 粗估只用于排序，**不是承诺**。要花钱的探测（M6）在设备端到端里一次跑完。
> `示例数据.md` 每轮只勾一家，**不要为了验证重跑 `ProtocolSpike`**（一轮约 30 个请求）。
...
### 4.1 M5 收尾：让探测真的跑起来并证明它是对的 ✅（实现完成，设备验收 09-07 已过）
...
### 4.3 M7 余额：数字必须和后台页面对得上 ✅（实现完成，设备验收 09-07 已过，真实样例待补）
...
| 6 | ~~填掉 M4 留下的 SAF 导出入口~~ 实际填的是**加密备份**（§12.1 `BackupEngine`）的 SAF 导入导出；M4 遗留的「`TextExporter` 反向导出」（§11.3 文本导出）是**可砍项**（`TextExporter` 注释自述），纯函数 + 单测已就绪，SAF 保存入口**未接**，与加密备份是两回事 |
...
### 4.6 M10 打磨与发布 ✅（发布链路完成，搜索/批量/更新页已落地；宽屏双栏已决定不做，TalkBack 待设备验收）

| 步骤 | 内容 |
| --- | --- |
| 1 | `audit_log` 持久化 + 脱敏 + 日志页（表已建）；日志条数与天数上限 |
| 2 | 搜索 / 排序 / 批量操作 ✅（2026-09-06 已落地）；~~宽屏双栏~~（已决定不做，2026-09-07） |
...
| 6 | `README.md`：功能、隐私声明；~~如实写明 6 位 PIN 挡不住离线穷举~~ **该段已随安全定位降级删除**（2026-09-08 重写为用户向版本），另建 `docs/index.html` 应用介绍落地页 |
...
| 项 | 步骤 | 何时做 |
| --- | --- | --- |
| **仪器测试** | `androidTest/` 现有 `VaultDatabaseTest`（6 用例：部分唯一索引 `idx_keys_default`、外键 CASCADE/SET_NULL）。**已修掉外键未开启的 bug**（`PRAGMA foreign_keys` 默认 OFF，CASCADE/SET_NULL 全是摆设）。`SecureFlagTest` 随迁移阶段1 删除 `FLAG_SECURE` 一并移除。还差：Room 迁移测试（升 v2 时补） | 设备上 connectedTest 已跑通 |
| ~~生物识别设备验证~~ | **作废**：双端迁移阶段1 已删除生物识别，无此功能可验 | — |
| **ADB 端到端** | M4 粘贴导入、M5 探测、M7 余额**均已在设备上跑通**（09-06 / 09-07）。**只剩 M6 客户端伪装**：先建客户端预设，再验 Agent Router 三条路径与 TLS 指纹终止结论 | 需设备 + 真实额度 |
| **额度耗尽真实响应** | DeepSeek 那 CNY 0.89 用完后补一次 `ProtocolSpike`，把响应加进 `fixtures/probe-matrix.json` | 余额自然耗尽时 |
| ~~SAF 导出入口~~ | ✅ **加密备份**的 SAF 导入导出已在 M9 填掉；`TextExporter` 反向导出（§11.3 文本导出）是可砍项，纯函数+单测就绪、SAF 入口未接 | 加密备份已完成 |
...
| ~~生物识别的设备验证~~ | **作废**：双端迁移阶段1 已删除生物识别 | — |
| iOS 真机验收 | 未签名 IPA 已能构建，未做过真机安装与全流程走查 | 需 Mac + 签名证书，见 `迁移计划.md` |
