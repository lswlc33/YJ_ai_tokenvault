...
> 设计细节、推导过程、完整红线条文仍在 `计划.md`；架构不变量与 36 条红线以
> `CLAUDE.md` 为准（**改代码前必须读**）。本文件不替代它们，也不与它们冲突；
> 若出现冲突，以 `CLAUDE.md` 为准。
>
> 状态截至 2026-09-06。代码基线：提交 `aed817f` + 一批未提交的 M5/M6 改动（M5 接线 + M6 客户端伪装已落地，尚未提交）。
...
| M1 安全底座 | ✅ | 生物识别无设备可验（当前设备无指纹硬件） |
| M2 数据层 | 🟡 基本完成 | 仪器测试未写；`ProfileSeeder` 未做；`SettingsRepository` 仅 1 项 |
| M3 接真数据 | ✅ | 无（管理四页 + 仪表盘六卡均吃真数据） |
| M4 文本导入 | 🟡 | ADB 端到端未跑；SAF 导出入口未做（可砍项） |
| M5 探测引擎 | 🟡 核心完成，接线收尾 | `ProbeEngine` 已落地并跑通；明细页未接真数据、未设备验收（工作区有一批未提交改动） |
| M6 客户端伪装 | 🟡 纯函数层 + 预设 UI + 嗅探已完成 | 未设备端到端（Agent Router 三路径 / TLS 指纹终止结论） |
| M7 余额 | ❌ | 全部未做（表与列已就绪） |
| M8 模型元数据 | ❌ | 全部未做（可砍，表已建） |
| M9 备份与同步 | ❌ | 全部未做 |
| M10 打磨与发布 | ❌ | 全部未做 |
| M11 可选增强 | ➖ | 逐项独立评估 |
...
- M0.5 打过三家真实中转站，五个答案 + 四条衍生事实写入 `计划.md` §16，脱敏 fixture 在 `app/src/test/resources/fixtures/`。
...
| 优先级 | 项 | 说明 | 粗估 |
| --- | --- | --- | --- |
| **P0-1** | M5 收尾 | 提交接线、补单测、删占位字段、设备验收 | 1 人日 |
| **P0-2** | M7 余额 | 6 个内置适配器，每个都要贴真实响应样例 | 3 人日 |
| **P1-1** | M8 模型元数据 | 可砍（不影响核心四件事） | 2 人日 |
| **P1-2** | M9 备份与同步 | 本地加密备份必做，WebDAV 半边可砍 | 4 人日 |
| **P1-3** | M10 打磨与发布 | 发布链路必做，宽屏双栏可砍 | 3 人日 |
| **P2** | 遗留补做 | 仪器测试、生物识别验证、ADB 端到端（含 M5/M6 探测与伪装）、额度耗尽采集 | 2 人日 |
| **P3** | M11 可选增强 | 供应商预设库 / Deep Link / SQLCipher / 桌面端 | 逐项评估 |

> 粗估只用于排序，**不是承诺**。M5–M7 建议连续做完（M5 与 M6 强耦合，探测与伪装要一起验）。

---

## 4. 实施路径

### 4.1 P0-1 · M5 收尾：让探测真的跑起来并证明它是对的
...
| 4 | **探测进行中暂停前台空闲锁定计时**（§7.4，`AutoLocker` 里目前留空）——否则长探测会被自己锁掉 |
...
| 2 | **`ProfileSeeder` 幂等种入 8 个内置预设**（M2 遗留项，预设内容见 `计划.md` §8.2 表；`zcode` 留空位） |
...
| 项 | 步骤 | 何时做 |
| --- | --- | --- |
| **仪器测试** | `androidTest/` 现在只有 `HiltTestRunner`。补：Room 迁移（含 `idx_keys_default` 部分唯一索引存在性）、级联删除、`BootStore` 原子写、`FLAG_SECURE`（含恢复密钥页）、WorkManager 锁定态行为 | M7 之后集中补 |
| **生物识别设备验证** | 换一台有指纹的机器，走：开启 → 解锁 → 改指纹后自动失效回退 PIN → 关闭后多次 PIN 解锁不悄悄打开 | 有设备时 |
| **ADB 端到端** | 插设备后跑 `AGENTS.md` 里的流程：粘贴 `示例数据.md` → 全量探测 → 余额 → 快速复制 | M7 完成后一次跑完 |
| **额度耗尽真实响应** | DeepSeek 那 CNY 0.89 用完后补一次 `ProtocolSpike`，把响应加进 `fixtures/probe-matrix.json` | 余额自然耗尽时 |
| **SAF 导出入口** | 与 M9 第 6 步合并做；不做也行（§11.3 是可砍项） | 随 M9 |
| **README / 文档** | M10 第 6 步 | 随 M10 |
...
1. 能构建、能跑测试、能装到设备上看到东西（`计划.md` §16 开头）。
...
| 36 条红线 | `CLAUDE.md`「36 条红线」，推导见 `计划.md` §3 |
| 数据层六条推论（Room 单例 / 锁定不关库 / DAO 不解密 / boot 原子写） | `CLAUDE.md`「六条推论」 |
| 加密边界（哪些列加密、哪些明文） | `CLAUDE.md`「加密边界」+ `计划.md` §4.4 |
| `KeyHealth` 与 `ProbeOutcome` 不是一个东西 | `CLAUDE.md`「两个状态列」 |
| 五条 CI grep 规则 | `计划.md` §14.4 |
| 完整验收清单（第 3 节每条红线的落地证明） | `计划.md` §17 |
