...
ui/shell    AppRoot / LockGate / VaultShell / Navigation 3 路由 / Routes
...
32. 落库前的脱敏**以已知明文值替换为主，正则只作兜底**——正则挡不住 base64 形态的访问令牌。
    ...
    **现状（2026-09-06）**：`ProbeEngine` 已接真对 `api_keys.healthDetail` 与明细页 detail 的
    `Redactor.scrub`；已知明文第一道由 `crypto/KnownSecrets` 提供——探测 reveal 与用户展开
    明文时登记、`VaultSession.lock()` 时清空，`provideRedactor` 用 `knownSecrets::snapshot`
    接线，正则只作兜底。
...
同步页的「备份口令」不做成独立设置页，改成**常驻提示文本**（`sync_passphrase_summary`），
消除了"点了没反应的箭头行"这个撑谎（§12.1 本就要求"提示常驻、不折叠"）。
